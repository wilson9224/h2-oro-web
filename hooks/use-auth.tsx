...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchProfile = useCallback(async (accessToken: string) => {
    try {
      ...
      if (authError || !authUser) throw new Error('No auth user');
  ...
  const signUp = async (data: SignUpData) => {
    ...
    // 2. Get the 'client' role ID. Role seeding must happen server-side
    // through migrations or an admin-only endpoint, never from the browser.
    ...
    if (roleError || !clientRole) {
      throw new Error('Error de configuración: rol de cliente no encontrado.');
    }
